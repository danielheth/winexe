...
const fs = require('fs'),
    ...
    readProperties = require('./read-properties'),
...
function WinExe(options) {
    ...
    this['authentication-file'] = options['authentication-file'];
...
WinExe.prototype._getArgsForWinExe = function () {
    ...
    if (this['authentication-file']) {
        console.log('using auth', this['authentication-file']);
        args.push('--authentication-file=' + this['authentication-file']);
    }
...
WinExe.prototype._getArgsForPsExec = function () {
    ...
    if (this['authentication-file']) {
        var creds = readProperties(this['authentication-file']);
        if (!('-u' in args)) {
            args.push('-u', creds.username);
        }
        var authFile = path.join(__dirname, './auth_password');
        fs.writeFileSync(authFile, creds.password);
        args.push('-p@', authFile);
        args.push('-p@d');
    }
...
WinExe.prototype._exec = function (callback) {
    ...
    cp.on('close', function (code) {
        ...
        try {
            if (self['authentication-file'] && fs.existsSync(self['authentication-file'])) {
                    fs.unlinkSync(self['authentication-file']);
            }
        } catch(e) {
            //ignore error
        }
